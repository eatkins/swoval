...
/**
 * Provides a platform dependent implementation of {@link RegisterableWatchService}. On osx, the
 * implementation will be {@link MacOSXWatchService}. On linux and windows, it will be an object
 * that implements {@link RegisterableWatchService} by delegation with an instance of {@link
...
public class RegisterableWatchServices {
  ...
  private static class RegisterableWatchServiceImpl
    ...
    public WatchKey register(final Path path, final Kind<?>... kinds) throws IOException {
      return watchService instanceof RegisterableWatchService
          ? ((RegisterableWatchService) watchService).register(path, kinds)
