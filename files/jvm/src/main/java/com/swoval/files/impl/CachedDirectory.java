...
interface CachedDirectory<T> extends UpdatableFileTreeView<T>, AutoCloseable {
   ...
   * {@link CacheEntry entry} for the path itself. When the maxDepth parameter is <code>-1
   ...
   */
  List<CacheEntry<T>> list(final int maxDepth, final Filter<? super CacheEntry<T>> filter);

  /**
   * Returns the {@link CacheEntry} associated with the path specified by {@link
